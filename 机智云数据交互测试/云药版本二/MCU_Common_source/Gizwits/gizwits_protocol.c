...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagvaluedata)
    {
        info->event[info->num] = EVENT_valuedata;
        info->num++;
        gizMemcpy((uint8_t *)dataPoints->valuevaluedata,issuedData->attrVals.valuevaluedata,sizeof(issuedData->attrVals.valuevaluedata));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(0 != memcmp((uint8_t *)&last->valuevaluedata,(uint8_t *)&cur->valuevaluedata,sizeof(last->valuevaluedata)))
    {
        GIZWITS_LOG("valuevaluedata Changed\n");
        ret = 1;
    }
    if(last->valuetemp_alarm != cur->valuetemp_alarm)
    {
        GIZWITS_LOG("valuetemp_alarm Changed\n");
        ret = 1;
    }
    if(last->valuehum_alarm != cur->valuehum_alarm)
    {
        GIZWITS_LOG("valuehum_alarm Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(temp_alarm_BYTEOFFSET,temp_alarm_BITOFFSET,temp_alarm_LEN,(uint8_t *)devStatusPtr,dataPoints->valuetemp_alarm);
    gizStandardCompressValue(hum_alarm_BYTEOFFSET,hum_alarm_BITOFFSET,hum_alarm_LEN,(uint8_t *)devStatusPtr,dataPoints->valuehum_alarm);
    ...
    gizMemcpy((uint8_t *)devStatusPtr->valuevaluedata,(uint8_t *)&dataPoints->valuevaluedata,sizeof(dataPoints->valuevaluedata));
    return 0;
}
