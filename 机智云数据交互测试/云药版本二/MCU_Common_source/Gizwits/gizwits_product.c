...
extern u8 wifi_sta;	// wifi状态标志位

extern uint8_t read_buff[50];	// 数据接受缓冲区

extern u8 read_sta; 	// 数据接受标志位 
  ...
  for(i=0; i<info->num; i++)
  {
    switch(info->event[i])
    {
      ...
      case EVENT_valuedata:
        ...
        //user handle
			 memcpy((uint8_t *)read_buff,(uint8_t *)&currentDataPoint.valuevaluedata,
      ...
      case WIFI_DISCON_ROUTER:
        ...
        break;
      case WIFI_CON_M2M:
			wifi_sta=1;		// wifi设备已连接
        break;
      case WIFI_DISCON_M2M:
			wifi_sta=0;		// wifi设备断开
        break;
      case WIFI_RSSI:
//        GIZWITS_LOG("RSSI %d\n", wifiData->rssi);	// 信息打印  暂时用不到
        break;
